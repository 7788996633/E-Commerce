...
class ProductItem extends StatelessWidget {
  const ProductItem({super.key, required this.poroductId});
  final String poroductId;
  @override
  Widget build(BuildContext context) {
    final productList =
        Provider.of<Products>(context, listen: false).productsItems;
    final curreProduct =
            ...
            trailing: IconButton(
              onPressed: () {
                if (curreProduct.quantity > 1) {
                  cart.addItem(curreProduct.id, curreProduct.title,
                      curreProduct.price, curreProduct.image);
                }
              },
